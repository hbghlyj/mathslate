...
*/
    ...
    // Standalone: original glyph came from a Moodle icon font; use a plain "?".
    var help = Y.Node.create('<button type="button" class="'
           + CSS.HELP + '" title="'
           + M.util.get_string('help', 'tinymce_mathslate') + '" style="font-weight:bold">?</button>');
    ...
    function makeDraggable () {
        ...
        if (se.getSelected() && preview.one('#' + se.getSelected())) {
            // MathJax 4 port (documented patch): slow async re-renders let
            // this run against the pre-render canvas, which may still lack
            // the freshly rekeyed selected node — guard instead of crashing.
            var selectedCanvasNode = canvas.get('node').one('#' + se.getSelected());
            if (selectedCanvasNode) {
                selectedCanvasNode.addClass(CSS.SELECTED);
                selectedCanvasNode.setAttribute('mathcolor', 'green');
                selectedCanvasNode.setAttribute('stroke', 'green');
                selectedCanvasNode.setAttribute('fill', 'green');
            }
