...
NS.Editor = function(editorID, config, params) {
    ...
    //Standalone patch: a config passed as a JavaScript object/array
    //(e.g. M.tinymce_mathslate.configJSON from js/config.js) is used
    //directly, so no XHR is needed and the app also runs from file://.
    //A string config is still treated as a URL and fetched with Y.io.
    var cfg = (typeof config !== 'undefined') ? config : NS.configJSON;
    if (cfg && typeof cfg === 'object') {
        MathJax.Hub.Queue(['fillToolBox', tbox, cfg, toolboxID]);
    } else {
        var request;
