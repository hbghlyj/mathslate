...
dimensions first — any **1×1 to 10×10** grid — then drops the cursor
into the first cell, so `Tab` fills the matrix cell by cell, row by
row (`Shift+Tab` steps back). The size is remembered for the session:
the next matrix click defaults to it, and *dragging* the tool onto the
slate inserts at that size straight away (a drag cannot pause for a
prompt mid-gesture). Typing `\matrix{a&b\\c&d}` into the TeX tab is
