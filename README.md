...
| `config.json` / `js/config.js` | The Latin-alphabet tab gains two quick-access rows alongside its lowercase/uppercase/digits rows: calligraphic script `\mathcal A`–`\mathcal Z` and Fraktur `\mathfrak A`–`\mathfrak Z` (26 + 26 single-glyph tools, built exactly like MathJax's own TeX output — `<mi mathvariant="script|fraktur">A</mi>` — so the canvas, the label and the TeX read-out all agree). `js/config.js` is the inlined runtime copy of `config.json`; both list the tools identically. |
